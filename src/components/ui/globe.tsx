...
export default function Globe({
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      return Math.max(Math.round(rect.width), 200);
    };

    const initGlobe = () => {
      const nextWidth = measure();
      if (nextWidth < 200) return;
      if (globeRef.current && nextWidth === lastWidth) return;
      ...
      try {
        ...
        markers: markers || [],
        scale: scale || 1,
        onRender: (state: { phi: number; theta: number; width: number; height: number }) => {
          if (!state) return;
  ...
  return (
    <div ref={containerRef} className={`relative ${className}`}>
      <canvas
        ...
        className='h-full w-full cursor-grab opacity-100'
